# ROADMAP — Manual do Vinho

Fila executável da obra: **13 volumes, 67 capítulos**, em 6 fases, do básico ao
avançado. Marque `[x]` ao concluir cada capítulo. Convenções de escrita estão no
`PLANO.md`; o gabarito de estilo é `v1-mundo-do-vinho/04-da-uva-a-taca.qmd`.

Regra da **fatia vertical**: feche e publique um volume inteiro antes de abrir o
próximo (descomentando seu bloco `part:` no `_quarto.yml`).

A coluna *fig.* sugere a figura TikZ didática de cada capítulo (esquemática;
fotos entram como imagem embutida — ver `PLANO.md`).
...
- [ ] `v1-mundo-do-vinho/01-o-que-e-vinho.qmd` — definição, composição, recorte do manual. *fig.: composição do vinho (pizza/blocos).*
- [ ] `v1-mundo-do-vinho/02-historia-origens.qmd` — Cáucaso, Egito, Grécia, Roma. *fig.: linha do tempo da Antiguidade.*
...
- [ ] `v3-castas/02-castas-tintas.qmd` — Cabernet, Merlot, Pinot Noir, Syrah, Tempranillo, Sangiovese, Malbec. *fig.: mapa casta × região.*
- [ ] `v3-castas/03-castas-brancas.qmd` — Chardonnay, Sauvignon Blanc, Riesling, Pinot Grigio, Chenin. *fig.: espectro aromático por casta.*
...
### Volume IV — Princípios da Vinificação
- [ ] `v4-principios-vinificacao/01-composicao-do-mosto.qmd` — açúcares, ácidos, fenólicos, taninos. *fig.: composição do mosto.*
- [ ] `v4-principios-vinificacao/02-fermentacao-alcoolica.qmd` — química, leveduras, curva. *fig.: curva de fermentação (açúcar↓/álcool↑).*
- [ ] `v4-principios-vinificacao/03-fermentacao-malolatica.qmd` — málico→lático. *fig.: esquema da conversão.*
...
- [ ] `v5-vinificacao/02-vinificacao-branco.qmd` — prensagem direta, fermentação a frio. *fig.: fluxograma do branco.*
...
- [ ] `v6-espumantes-fortificados/02-charmat-e-outros.qmd` — autoclave, ancestral, transfer. *fig.: Charmat × tradicional.*
- [ ] `v6-espumantes-fortificados/03-fortificados.qmd` — Porto, Jerez (solera), Madeira. *fig.: sistema de solera.*
...
## Fase 4 — Estilos e regiões

### Volume VIII — Estilos e Tipos de Vinho
- [ ] `v8-estilos/01-tintos.qmd` — do leve ao encorpado. *fig.: eixo corpo × tanino.*
- [ ] `v8-estilos/02-brancos.qmd` — secos, aromáticos, com madeira. *fig.: eixo acidez × aroma.*
...
- [ ] `v13-cultura-mercado/01-economia-e-investimento.qmd` — mercado, preço, leilões, investimento. *fig.: cadeia de valor do vinho.*
...
## Checklist ao fechar cada volume
- [ ] Todos os capítulos do volume com `[x]`.
- [ ] Bloco `part:` do volume ativo no `_quarto.yml`.
- [ ] Termos novos registrados no `glossario.qmd`.
- [ ] Figuras com `%%| alt:` preenchido (acessibilidade).
- [ ] `quarto render` sem erros (HTML + PDF).
- [ ] Grafo do `index.qmd` revisado se as dependências mudaram.
