# Método de figuras (TikZ → SVG/PDF) para manuais Quarto

> **Arquivo autocontido.** Cole este arquivo inteiro numa conversa com a IA (ou
> ponha-o na raiz do novo repositório e aponte a IA para ele). Ele contém o
> passo a passo **e** o código integral dos dois arquivos da extensão
> (`tikz.lua` e `_extension.yml`, nos Anexos A e B), para que a IA possa recriar
> tudo do zero, byte a byte.

## O que é o método

A figura é escrita em **TikZ/PGFplots** dentro do próprio `.qmd` — texto
versionável, sem arquivos de imagem soltos. Um filtro Lua (`danmackinlay/tikz`,
com patches locais) intercepta cada bloco ```` ```{.tikz} ```` e compila para
**SVG embutido inline** (no HTML) ou **PDF embutido** (no livro PDF).

## Passo a passo para portar a um manual novo

### 1. Criar os arquivos da extensão

Crie a pasta `_extensions/danmackinlay/tikz/` no repositório e, dentro dela, dois
arquivos com o conteúdo **integral** dos anexos deste documento:

- `_extensions/danmackinlay/tikz/_extension.yml`  → **Anexo A**
- `_extensions/danmackinlay/tikz/tikz.lua`        → **Anexo B**

> ⚠️ Use o `tikz.lua` do **Anexo B** (tem patches locais para Windows, saída PDF
> e SVG inline). **Não** rode `quarto add`/`quarto update` para baixar o upstream
> — ele vem sem os patches e quebra de forma silenciosa.

### 2. Configurar o `_quarto.yml`

```yaml
filters:
  - danmackinlay/tikz   # TEM de vir ANTES de `quarto`, senão @fig-… não numera
  - quarto
...
### 3. Instalar o toolchain local (precisa de TeX, mesmo só para HTML)

```bash
quarto install tinytex
# depois, no bin da plataforma do TinyTeX (ex.: bin/windows):
tlmgr install standalone pgf pgfplots dvisvgm xcolor amsmath amsfonts
```

Cada figura é compilada por `pdflatex → dvisvgm`. Numa máquina **sem TeX**, o
`quarto preview` quebra em qualquer capítulo com figura.

### 4. CI (GitHub Actions) — instalar os pacotes LaTeX no runner

O filtro chama `pdflatex`/`dvisvgm` direto (fora do latexmk do Quarto), então o
TinyTeX do runner precisa destes pacotes:
...
```yaml
      ...
      - name: Instalar pacotes LaTeX das figuras TikZ
        run: |
          TLMGR="$(find / -name tlmgr 2>/dev/null | head -1)"
          test -n "$TLMGR" || { echo "tlmgr nao encontrado"; exit 1; }
          echo "$(dirname "$TLMGR")" >> "$GITHUB_PATH"
          "$TLMGR" install standalone pgf pgfplots dvisvgm xcolor amsmath amsfonts
```

### 5. Escrever uma figura num `.qmd`
...
- O **id (`#fig-nome`) vai na `div`**, não no bloco de código.
- A **legenda** é o parágrafo logo antes do `:::`.
- Referencie sempre com `@fig-nome` (nunca "Figura 4.1" na mão).
- Preâmbulo e paleta são **centralizados** no template dentro do `tikz.lua` —
  não repita `\usepackage` por figura.
- Estilos prontos: `curva` (azul grosso), `destaque` (vermelho), `auxiliar`
  (cinza tracejado), `eixo`, `ponto`, `vetor`. Cores: `manualblue`, `manualred`,
  `manualgreen`, `manualyellow`, `manualgray`. Ajuste a paleta no template à
  identidade do novo manual, se quiser.

## Resumo dos patches locais do `tikz.lua` (Anexo B)

Marcados com `(Local patch...)` no código. Reaplique se um dia atualizar a
extensão:

1. **`check_dependency`** — usa `pandoc.pipe(cmd, {'--version'})` em vez de
   `command -v`, que falha no `cmd.exe` do Windows.
2. **Caminhos absolutos na compilação** — no Windows o subprocesso não herda o
   `chdir`; usa paths absolutos para `.tex`/`.pdf`/`.svg`/`.dvi`.
3. **DVI só quando converte por dvisvgm** — `-output-format=dvi` apenas no modo
   SVG; em PDF mantém a saída PDF.
4. **`build_texinputs` detecta Windows por `package.config`** — `pandoc.system.os`
   retorna `mingw32` no Windows e quebrava `C:` no TEXINPUTS.
5. **SVG inline + namespacing de ids** — em HTML o SVG é embutido inline (não em
   `<pagina>_files/`), porque o `quarto publish gh-pages` derrubava esses
   arquivos (404). Cada id/href/url(#…) é prefixado com `tz_<basename>_`.
6. **Preâmbulo + paleta no template `standalone` embutido** — pacotes, cores e
   estilos centralizados; e a linha `\ifdefined\pdfoutput…` faz a mesma fonte
   servir para HTML (driver dvisvgm em DVI) e PDF (pgf detecta pdftex sozinho).


## Anexo A — `_extensions/danmackinlay/tikz/_extension.yml`
...
## Anexo B — `_extensions/danmackinlay/tikz/tikz.lua`

Copie o conteúdo abaixo **integralmente** para `_extensions/danmackinlay/tikz/tikz.lua`.
